...
ConvertTo-Html -Title "System Information for $vComputerName_report" -Body " <br> <center> <div style=""width:1200px;height:1100px;border:2px solid #6a99a8;""> <div style=""width:1199px;height:115px;border:1px solid #5a7c87;""><br> <!-- logo de votre organisation  --><img src=""https://www.lyon-metropole.cci.fr/plugins/CciWebPlugin/images/logo_CCI-Lyon-Metropole-Saint-Etienne-Roanne.png"" width=200 align=""top"" /> <center> <h1> Status Report for : $vComputerName_report </h1>" >  "$filepath\Desktop\$vComputerName_report.html" 
...
#################################################
Get-WMIObject -class Win32_ComputerSystem | select Domain, PrimaryOwnerName, Manufacturer, Model, UserName | ConvertTo-html -Body "<H3> Network Organization & more  </H3>" >>  "$filepath\Desktop\$vComputerName_report.html"
...
Get-WmiObject win32_logicalDisk -ComputerName $vComputerName_report | select DeviceID,VolumeName,FileSystem, ProviderName,@{Expression={$_.Size /1Gb -as [int]};Label="Total Size(GB)"},@{Expression={$_.Freespace / 1Gb -as [int]};Label="Free Size (GB)"} `
...
Get-WmiObject Win32_PhysicalMemory |
        ...
        Speed, TotalWidth | ConvertTo-html -Body "<h3> RAM Information </h3>" >>  "$filepath\Desktop\$vComputerName_report.html"

################################################
#  footer   
#################################################

$Report = " <br> <br> <br> <strong> <i> <h4>The report was generated On  $(Get-Date -Format g) by <u> $((Get-Item env:\username).Value) </u> (post deployment) 
</h4> </i> </strong> "
$Report  >> "$filepath\Desktop\$vComputerName_report.html" 
$report2 = " <h4> All rights reserved - brlndtech © (FR) | check the code ? <a href =""https://github.com/brlndtech/windows-pc-report"" target=""_blank""> check my github </a> <br> </strong> </div> </center> "
